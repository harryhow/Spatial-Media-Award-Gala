...
void testApp::loadSettings() {
    ...
    // TODO: need to define in XML
    bShowExpression = true;
...
template <>
void testApp::addMessage(string address, string data) {
	ofxOscMessage msg;
	msg.setAddress(address);
...
void testApp::update() {
	...
	if(videoSource->isFrameNew()) {
		if (tracker.update(toCv(*videoSource))){
		...
		if(tracker.getFound()) {
            ...
            if (classifier.size() > 0){
                addMessage("/expression/primary", classifier.getDescription(classifier.getPrimaryExpression()));
            }
		} else {
			addMessage("/found", 0);
		}
...
void testApp::draw() {
	...
	if(tracker.getFound()) {
        ...
        //if (bShowExpression){
            ...
            //ofLog(OF_LOG_NOTICE, "n=%d",n);
            int primary = classifier.getPrimaryExpression();
            for(int i = 0; i < n; i++){
                ofSetColor(i == primary ? ofColor::red : ofColor::black);
                ofRect(10, 40, w * classifier.getProbability(i) + .5, h);
                ofSetColor(255);
                ofDrawBitmapString(classifier.getDescription(i), 5, 9);
                ofTranslate(0, h + 5);
                
                cout << "decription=" << classifier.getDescription(i) << endl;
            }
            ofPopMatrix();
            ofPopStyle();
        //}
